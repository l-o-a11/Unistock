...
const barIcons = {
  ...
  "Tráfico entre sedes":
    "M1 3h15v13H1zM16 8h4l3 3v5h-7V8zM5.5 19a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5zm13 0a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z",
  "Ficha técnica":
    "M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M16 13H8M16 17H8",
  Corte:
    "M6 3a3 3 0 1 0 0 6 3 3 0 0 0 0-6zm0 12a3 3 0 1 0 0 6 3 3 0 0 0 0-6zM20 4 8.12 15.88M14.47 14.48 20 20M8.12 8.12 12 12",
...
const calcAvgDays = (orders) => {
  ...
  if (!validCount) {
    if (invalidSamples.length) {
      console.warn(
        '[Dashboard] Tiempo promedio: ninguna orden "Enviado" tiene fechas válidas. Ejemplos:',
        invalidSamples,
      );
    }
    return null;
...
// ✅ Extraído fuera de ProductionDashboard: definir un componente dentro de
// otro componente hace que se recree (y pierda estado interno, como el del
// Dropdown) en cada render. Recibe todo lo que necesita por props.
function GlobalTimeFilter({
  timeView,
  setTimeView,
  selectedMonth,
  setSelectedMonth,
...
export default function ProductionDashboard() {
  ...
  const modalRangeError =
    !!(modalDateFrom && modalDateTo) &&
    new Date(modalDateFrom) > new Date(modalDateTo);
  ...
  const sedeSeriesNames = useMemo(() => {
    ...
    orders.forEach((order) => {
      (order.sedeAsignaciones || order.sede_asignaciones || []).forEach(
        (assignment) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const [ordersRequest, sedesRequest, suppliesRequest] =
        ...
        if (ordersRequest.status === "fulfilled") {
          setOrders(
            Array.isArray(ordersRequest.value) ? ordersRequest.value : [],
          );
        } else {
          console.error(
            "[Dashboard] No se pudieron cargar las producciones:",
            ordersRequest.reason,
          );
        }

        const sedesResult =
          sedesRequest.status === "fulfilled" ? sedesRequest.value : null;
        const suppliesResult =
          suppliesRequest.status === "fulfilled" ? suppliesRequest.value : null;
        setSedesNames(
          (sedesResult?.data || []).map((s) => s.nombre).filter(Boolean),
        );
        const allSupplies = Array.isArray(suppliesResult?.data)
          ? suppliesResult.data
          : Array.isArray(suppliesResult)
            ? suppliesResult
            : [];
        setSupplies({
          adquisicion: allSupplies.filter((s) => (s.stock ?? 0) === 0).length,
          almacenamiento: allSupplies.length,
          stock: allSupplies.reduce(
  ...
  const { stats, generalStatus } = useMemo(() => {
    ...
    // eslint-disable-next-line react-hooks/purity -- se necesita el "ahora" real para calcular retrasos
    ...
    const inPeriodActive = (o) => {
      if (matchPeriod(o.updatedAt, timeView, monthIdx, selectedYear))
        return true;
      if (matchPeriod(o.createdAt, timeView, monthIdx, selectedYear))
        return true;
      return (o.historial || []).some((h) =>
        matchPeriod(h.fecha, timeView, monthIdx, selectedYear),
      );
    };

    // ── Producciones actuales: estado "Producción" con actividad en el período ──
    const currentInPeriod = orders.filter(
      (x) => ESTADOS_EN_PRODUCCION.includes(x.estado) && inPeriodActive(x),
    ).length;
    const currentTotal = orders.filter((x) =>
      ESTADOS_EN_PRODUCCION.includes(x.estado),
    ).length;
    ...
    const prevDone = orders.filter((x) => {
      ...
      if (timeView === "Año")
        return new Date(d || 0).getFullYear() === selectedYear - 1;
    ...
    active.forEach((x) => {
      ...
      if (fe) {
        ...
        if (!isNaN(ms) && nowMs > ms) isDelayed = true;
      }
      if (
        !isDelayed &&
        ESTADOS_EN_PRODUCCION.includes(x.estado) &&
        (x.asignaciones || []).length > 0
      ) {
        const entrada = (x.historial || []).find((h) =>
          ESTADOS_EN_PRODUCCION.includes(h.estado),
        );
        const fechaEntrada = entrada?.fecha || x.updatedAt;
        if (fechaEntrada) {
          const dias = Math.round(
            (nowMs - new Date(fechaEntrada).getTime()) / 86400000,
          );
    ...
    return {
      stats: {
        ...
        completedThisMonth,
        pending,
        avgTime: avgDays !== null ? `${avgDays}d` : "—",
        periodLabel,
        avgPeriodLabel,
      },
  ...
  const generalMaxVal = Math.max(
    generalStatus.delayed,
    generalStatus.onTrack,
    1,
  );
  const delayedBarPx =
    generalStatus.delayed > 0
      ? Math.max(
          6,
          Math.round(
            (generalStatus.delayed / generalMaxVal) * GENERAL_BAR_MAX_PX,
          ),
        )
      : 4;
  const onTrackBarPx =
    generalStatus.onTrack > 0
      ? Math.max(
          6,
          Math.round(
            (generalStatus.onTrack / generalMaxVal) * GENERAL_BAR_MAX_PX,
          ),
        )
      : 4;
  ...
  const lineData = useMemo(() => {
    ...
    const matchPoint = (d, pointIndex) => {
      if (!d) return false;
      if (timeView === "Año")
        return d.getMonth() === pointIndex && d.getFullYear() === selectedYear;
      if (timeView === "Mes")
        return (
          Math.ceil(d.getDate() / 7) === pointIndex + 1 &&
          d.getMonth() === monthIdx &&
      ...
      orders.forEach((o) => {
        ...
        if (o.estado !== "Anulada" && asigsSede.length > 0) {
          asigsSede.forEach((a) => {
            if (a?.option) {
              point[a.option] =
                (point[a.option] || 0) + (Number(a.cantidad) || 0);
            }
          });
        }
      });
  ...
  const handleDownloadExcel = async (dateFrom, dateTo) => {
    ...
    try {
      ...
      const periodoTexto = hasRange
        ...
        : timeView === "Semana"
          ? `${selectedMonth} ${selectedYear} (semana actual)`
          : timeView === "Mes"
      ...
      if (logoImageId)
        wsResumen.addImage(logoImageId, {
      ...
      wsResumen.getCell("B1").value =
        "Dashboard de Producción — Sistema de Gestión UniStock";
      wsResumen.getCell("B1").font = {
        name: "Arial",
        size: 15,
        bold: true,
        color: { argb: "000000" },
      };
      wsResumen.getCell("B1").alignment = {
        horizontal: "left",
        vertical: "middle",
        indent: 1,
      };
      ["A1", "B1"].forEach((ref) => {
        wsResumen.getCell(ref).fill = fillSolid("#FFFFFF");
      });

      wsResumen.mergeCells("B2:D2");
      wsResumen.getRow(2).height = 18;
      wsResumen.getCell("B2").value =
        `Generado el ${fechaGeneracion}  ·  Período: ${periodoTexto}`;
      wsResumen.getCell("B2").font = {
        name: "Arial",
        size: 10,
        color: { argb: "#000000" },
      };
      wsResumen.getCell("B2").alignment = {
        horizontal: "left",
        vertical: "middle",
        indent: 1,
      };
      ...
      const kpiRows = [
        ...
        [
          stats.periodLabel
            ? `Completadas ${stats.periodLabel}`
            : "Completadas",
          stats.completedThisMonth || 0,
        ],
        ["Por iniciar", stats.pending || 0],
        [
          stats.avgPeriodLabel
            ? `Tiempo promedio (${stats.avgPeriodLabel})`
            : "Tiempo promedio",
          stats.avgTime || "—",
        ],
      ...
      wsResumen.getCell(`A${procTitleRowIdx}`).font = {
        name: "Arial",
        size: 12,
        bold: true,
        color: { argb: "000000" },
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                <p style={{ margin: "3px 0 0", fontSize: 12, color: "#888" }}>
                  Selecciona un rango de fechas para el reporte
                </p>
              </div>
              <button
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              >
                Si no seleccionas fechas, se usará el período activo del
                dashboard.
              </p>
            </div>
            ...
            >
              ...
              <button
                onClick={() => {
                  ...
                  handleDownloadExcel(
                    isRange ? modalDateFrom : null,
                    isRange ? modalDateTo : null,
                  );
                }}
                disabled={
                  downloadingExcel ||
                  (modalDateFrom && modalDateTo && modalRangeError)
                }
                style={{
                  padding: "8px 16px",
                  borderRadius: 8,
      ...
      <Card className="mb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={label}
              className="rounded-xl px-3 sm:px-4 py-3 bg-white min-w-0"
              style={{ border: "1.5px solid #e5e7eb" }}
            >
              <div className="flex items-center gap-2 mb-2">
                <div
                  className={`w-6 h-6 rounded-md border flex items-center justify-center shrink-0 ${color}`}
                >
                  {icon}
                </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-4">
        <Card className="flex-1 min-w-0">
          <h2 className="text-xl font-bold text-gray-900 mb-4">
            Productos asignados a las sedes
          </h2>
              ...
              {sedeSeriesNames.map((name, i) => (
                ...
                >
                  <span
                    ...
                    style={{ background: SEDE_COLORS[i % SEDE_COLORS.length] }}
                  />
                  {name}
                </button>
              ))}
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span>
                ...
                <span className="font-semibold text-fuchsia-600">
                  {timeView === "Semana"
                    ? `${selectedMonth} ${selectedYear} — por semana`
                    : timeView === "Mes"
                      ? `${selectedMonth} ${selectedYear}`
                      : `Año ${selectedYear}`}
                </span>
              ...
              {sedeSeriesNames.map((name, i) =>
                viewMode === "Todas" || viewMode === name ? (
                  <Line
                    ...
                    type="monotone"
                    dataKey={name}
                    stroke={SEDE_COLORS[i % SEDE_COLORS.length]}
          ...
          <Card className="flex-1">
            ...
            <div className="flex items-end justify-around h-48">
              <div className="flex flex-col items-center gap-1">
                ...
                <div
                  className="w-10 rounded-lg bg-pink-300"
                  style={{
                    height: delayedBarPx,
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <div
                  className="w-10 rounded-lg bg-green-400"
                  style={{
                    height: onTrackBarPx,
                    transition: "height 0.3s ease",
                  }}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-pink-50 border border-pink-200 flex items-center justify-center shrink-0">
                    ...
                    >
                      ...
                      />
                      <path
                        d="M6.16748 9.53595V8.64309C6.16748 6.5119 7.579 4.468 10.0915 2.96102C12.604 1.45403 16.0118 0.607422 19.565 0.607422C23.1182 0.607422 26.5259 1.45403 29.0385 2.96102C31.551 4.468 32.9625 6.5119 32.9625 8.64309V9.53595"
                        stroke="#e040b8"
                        strokeWidth="1.21519"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span
                    className={`text-sm font-bold ${supplies.adquisicion > 0 ? "text-red-500" : "text-gray-900"}`}
                  >
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-fuchsia-50 border border-fuchsia-200 flex items-center justify-center shrink-0">
                    ...
                    >
                      <path
                        d="M39.6436 0.607422H3.86044C2.06385 0.607422 0.607422 1.47356 0.607422 2.542V23.8224C0.607422 24.8908 2.06385 25.757 3.86044 25.757H39.6436C41.4402 25.757 42.8966 24.8908 42.8966 23.8224V2.542C42.8966 1.47356 41.4402 0.607422 39.6436 0.607422Z"
                        stroke="#e040b8"
                        strokeWidth="1.21519"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      />
                      <path
                        d="M28.2572 0.607422V10.2803C28.2572 10.5369 28.0858 10.7829 27.7808 10.9643C27.4757 11.1457 27.062 11.2476 26.6307 11.2476H16.8716C16.4402 11.2476 16.0265 11.1457 15.7215 10.9643C15.4165 10.7829 15.2451 10.5369 15.2451 10.2803V0.607422"
                        stroke="#e040b8"
                        strokeWidth="1.21519"
                        strokeLinecap="round"
                        strokeLinejoin="round"
      ...
      <Card>
                ...
                >
                  {barData.map((entry, i) => {
                    ...
                    return (
                      <Cell
                        key={i}
                        fill={entry.value >= limit ? "#E8B4E8" : "#7BE87B"}
                      />
                    );
                  })}
                </Bar>
        ...
        <div className="flex items-center gap-2 mt-3 px-4 py-3 rounded-xl border border-fuchsia-200 bg-fuchsia-50">
          <div
            ...
            style={{ background: "#E8B4E8", border: "1.5px solid #d946ef" }}
          />
          <p className="text-xs text-fuchsia-700 font-medium">
            Los procesos en <span className="font-bold">rosa</span> superan el
            límite del período (día ≥ 10 · mes ≥ 24 · año ≥ 38). Pueden requerir
            atención de capacidad.
          </p>
        </div>
      </Card>
    </div>
