...
const SedesPage = () => {
    ...
    showAlert(
      ...
      async (pwd) => {
        ...
        try {
          ...
          showAlert("error", "Error", err.message || "No se pudo eliminar la sede.");
        }
      }
    );
  };
    ...
    showAlert(
      ...
      async (pwd) => {
        ...
        try {
          ...
          showAlert("error", "Error", err.message || `No se pudo ${accion} la sede.`);
        }
      }
    );
  };
  ...
  const handleCreateSubmit = async (data) => {
    try {
      ...
      showAlert("error", "Error al crear", err.message || "No se pudo crear la sede.");
    }
  };
  ...
  const handleEditSubmit = async (data) => {
    try {
      ...
      showAlert("error", "Error al actualizar", err.message || "No se pudo actualizar la sede.");
    }
  };
            ...
            <div style={{ padding: "20px 24px 24px" }}>
              {[
                ...
                <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #f5f5f5" }}>
                  ...
                  <span style={{ fontSize: "13px", fontWeight: label === "Estado" ? 600 : 400,
                    color: label === "Estado" ? (selectedSede.estado !== false ? "#22c55e" : "#ef4444") : "#111" }}>
                    {value}
                  </span>
                </div>
              ))}
