...
export default function Sidebar() {
  ...
  // ── Responsive tokens ──────────────────────────────────────────────────────
  const collapsedWidth = "clamp(72px, 5vw, 96px)";
  const expandedWidth = "clamp(200px, 15vw, 260px)";
  const iconSize = "clamp(22px, 1.8vw, 30px)";
  const logoSize = "clamp(36px, 3vw, 52px)";
  // ───────────────────────────────────────────────────────────────────────────
  ...
  if (loading) {
    return (
      <div style={{ width: collapsedWidth, height: "100vh", background: "#fff", borderRight: "1px solid #f3f4f6", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
  ...
  return (
    <>
      <style>{`
        ...
        .sb-label { font-size:clamp(12px, 0.9vw, 14px); font-weight:600; color:#374151; white-space:nowrap; overflow:hidden; flex:1; max-width:200px; transition:max-width 0.18s ease, opacity 0.18s ease; }
        ...
        .sb-subitem { border:none; cursor:pointer; background:transparent; border-radius:8px; display:flex; align-items:center; gap:10px; padding:9px 12px; width:100%; text-align:left; font-size:clamp(11px, 0.85vw, 13px); color:#9ca3af; font-weight:500; transition:background 0.1s ease, color 0.1s ease; }
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", padding: "18px 16px 10px", gap: 10, minHeight: 68 }}>
          <div style={{ width: logoSize, height: logoSize, borderRadius: 12, overflow: "hidden", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <img src={logo} alt="Logo" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          </div>
          <div style={{ overflow: "hidden", maxWidth: expanded ? "clamp(100px, 10vw, 160px)" : 0, opacity: expanded ? 1 : 0, transition: "max-width 0.18s ease, opacity 0.18s ease", whiteSpace: "nowrap" }}>
            <p style={{ fontSize: "clamp(15px, 1.2vw, 20px)", fontWeight: 700, color: "#111827", margin: 0, lineHeight: 1.2 }}>Putongas</p>
          </div>
        </div>
          ...
          {visibleMenuItems.map((item) => {
            ...
            return (
              <div key={item.id}>
                ...
                >
                  <Icon style={{ width: iconSize, height: iconSize, flexShrink: 0, color: iconColor(isActive), transition: "color 0.12s ease" }} />
                ...
                {item.hasSubmenu && (
                  <div
                    ...
                    style={{
                      maxHeight: isOpen && expanded ? item.submenu.length * 40 + "px" : "0px",
