...
const IconInput = ({ icon, type = "text", value, onChange, placeholder, disabled, className = "" }) => (
  <div className={`flex items-center gap-2.5 px-3 py-2 rounded-xl border transition-all bg-gray-50
    ${disabled
...
const PasswordInput = ({ value, onChange, placeholder, show, onToggle, hasError, hasSuccess }) => (
  <div className={`flex items-center gap-2.5 px-3 py-2 rounded-xl border transition-all bg-gray-50 focus-within:bg-white
  ...
  return (
    <div className="h-fit bg-gray-50">
      ...
      <div className="px-10 pt-7 pb-7">
        ...
        <p className="text-xs text-gray-400 mt-0.5">Administra tu información personal y seguridad</p>
      </div>
        ...
        <SectionCard title="Información personal" subtitle="Actualiza tu nombre y datos de contacto">
          <div className="grid grid-cols-2 gap-6 mb-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-1 ml-1">No se puede modificar</p>
            </div>
          </div>

          {profileError && <p className="text-xs text-red-500 font-medium mb-3">⚠ {profileError}</p>}
          {profileSuccess && <p className="text-xs text-green-500 font-medium mb-3">✓ {profileSuccess}</p>}
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-6 py-2 rounded-xl text-sm font-semibold bg-pink-500 hover:bg-pink-600 text-white shadow-md shadow-pink-100 transition-all duration-200 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
            >
        ...
        <SectionCard title="Cambiar contraseña" subtitle="Usa una contraseña de al menos 8 caracteres">
          <div className="grid grid-cols-3 gap-6 mb-4">
            ...
            <div>
              ...
              />
              {passwordsMismatch && <p className="text-xs text-red-400 mt-1 ml-1">Las contraseñas no coinciden</p>}
              {passwordsMatch && <p className="text-xs text-green-500 mt-1 ml-1">✓ Las contraseñas coinciden</p>}
            </div>
          </div>

          {/* Password strength rules — only when typing */}
          {newPassword.length > 0 && (
            <div className="grid grid-cols-3 gap-x-4 gap-y-1 mb-4 px-4 py-3 bg-gray-50 rounded-xl border border-gray-100">
              {passwordRules.map((rule) => (
                <span key={rule.label} className={`text-xs flex items-center gap-1.5 ${rule.ok ? "text-green-500" : "text-gray-400"}`}>
          ...
          {passwordError && <p className="text-xs text-red-500 font-medium mb-3">⚠ {passwordError}</p>}
          {passwordSuccess && <p className="text-xs text-green-500 font-medium mb-3">✓ {passwordSuccess}</p>}
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-6 py-2 rounded-xl text-sm font-semibold border-2 border-pink-500 text-pink-500 hover:bg-pink-500 hover:text-white transition-all duration-200 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
            >
